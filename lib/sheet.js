...
// Headshots are square but the card crops them to 16:10, so each person picks
// which band of their photo shows: 0% is the top of the image, 100% the bottom.
// The sheet may hold "25", "25%" or nothing. 25% is the default because it
// suits a standard head-and-shoulders portrait — eyes on the upper third —
// where dead centre cuts the forehead. Anything unparseable falls back to it
// too, rather than throwing a bad value into the stylesheet.
const DEFAULT_PHOTO_POSITION = '25%';

function normalizePercent(value) {
  const n = parseFloat(String(value || '').replace('%', '').trim());
  if (!Number.isFinite(n)) return DEFAULT_PHOTO_POSITION;
...
const FIELD_MATCHERS = [
  ...
  ['notes',         (h) => h === 'notes'],
  ['photo',         (h) => h.startsWith('photo') && !h.includes('position')],
  ['photoPosition', (h) => h.startsWith('photoposition')],
...
function rowsToPeople(rows, logoRows) {
  ...
  for (const row of rows.slice(1)) {
    ...
    const person = {
      ...
      photoPosition: normalizePercent(get('photoPosition')),
